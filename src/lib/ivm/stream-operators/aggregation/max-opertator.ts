...
type Result = { max: number };

/* stateful */
export class MaxOperator<T> implements Sink<T>, Source<Result> {
	...
	constructor(source: Source<T>, options: Options<T>) {
		...
		// sort by column we care about, desc
		this.#records = new BTree<T>((a, b) => b[options.column] - a[options.column]);
	}
	...
	get max() {
		return this.#maxRecord?.[this.#options.column];
	}
	...
	*pull() {
		if (!this.#initialized) {
			// clear in case we got any pushes before the first pull
			this.#records.clear();
		...
		yield [{ max: this.max }, 1] as [Result, number];
	}
	...
	#processRow(row: T, weight: number): boolean {
		...
		assert(
			typeof value === 'number',
			Error(`can only 'max' numbers - column: ${String(this.#options.column)} value: ${value}`)
		);
		if (weight > 0) {
