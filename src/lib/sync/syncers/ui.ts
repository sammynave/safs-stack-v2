...
import type { DatabaseInterface, RepoDefinition, BoundCommands } from '../types.ts';
...
export class Ui {
	...
	refreshFns: Map<string, { fn: () => void | Promise<void>; watchedTables: Set<string> }> =
		new Map();
	repos = new WeakMap<RepoDefinition, ReactiveRepo>();
	updateQueue = new Set<string>();
	queueing: Promise<void> | null = null;
	sync: (opts: { source: Source; watchedTables: Set<string> }) => Promise<void> = async () => {
	...
	repoFor<TView>(definition: RepoDefinition<TView>): ReactiveRepo<TView> {
		...
		const repoId = nanoid();
		const boundCommands = commands ? this.bindCommands(commands, new Set(watch)) : undefined;
	 ...
	 */
	async update({ watchedTables }: { watchedTables: Set<string> }): Promise<void> {
	...
	private bindCommands(
		commands: {
			[key: string]: (db: DatabaseInterface, args: Record<string, unknown>) => Promise<void>;
		},
		watch: Set<string>
	): BoundCommands {
