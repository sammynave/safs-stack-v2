...
import type { DatabaseInterface, DatabaseConnectionConfig, Change, Changes } from './types.ts';
import { CrdtEvent } from './types.ts';
import { generateSiteId } from './utils.ts';
import {
	SELECT_VERSION,
	SELECT_SITE_ID,
...
import { INIT_STATEMENTS } from './schema.ts';

export class DatabaseConnection {
	static async init(config: DatabaseConnectionConfig): Promise<DatabaseConnection> {
	 ...
	 */
	async lastTrackedVersionFor(siteId: string, event: CrdtEvent): Promise<number> {
	 ...
	 */
	async insertTrackedPeer(siteId: string, version: number, event: CrdtEvent): Promise<void> {
	 ...
	 */
	private async _mergeTableChanges(table: string, changes: Changes): Promise<void> {
		for (const change of changes) {
			const [tableName, pk, cid, val, col_version, db_version, site_id, cl, seq] = change;
		...
		for (const table of this.tables) {
         ...
         GROUP BY "pk", "cid"`,
			...
			for (const [pk, columns] of rowsByPk) {
				...
				const updates = columnNames.map((col) => `${col} = excluded.${col}`).join(', ');
