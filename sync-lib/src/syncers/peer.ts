...
import type { Transport, Changes } from "../types.js";
import { CrdtEvent, MessageType } from "../types.js";
...
export class Peer {
  ...
  async handleConnected({
    ...
    const sinceVersion = await this.dbConn.lastTrackedVersionFor(
      siteId,
      CrdtEvent.sent
    );
  ...
  async handleAck({
    ...
    const currentVersion = await this.dbConn.lastTrackedVersionFor(
      siteId,
      CrdtEvent.sent
    );
    ...
    if (currentVersion < version && this.serverSiteId) {
      await this.dbConn.insertTrackedPeer(
        this.serverSiteId,
        version,
        CrdtEvent.sent
      );
